...
brief). Thin httpx wrapper, deliberately no SDK. The API key rides the URL
(Google's own convention for this API), not a header.

Refusal/safety blocks come back as an empty `candidates` list with
`promptFeedback.blockReason` set, or a candidate with `finishReason` of
SAFETY/RECITATION/OTHER — reported as data (never raised), same contract
as the OpenAI client so the War Room's three seats handle refusal
