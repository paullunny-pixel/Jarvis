...
- {{"action":"research","account":"<inbox words or 'all'>","query":"<comma-separated search terms covering EVERY topic he named — e.g. he asks about BMI, EUDAMED and production registration → 'BMI, EUDAMED, production registration'. One term only finds emails containing that term, so a lone company name MISSES topic threads>","instruction":"<what Paul wants produced from those emails, restated in full>"}}   (read/summarise/dig through ALL email about a sender or topic — 'read all my emails from X and...', 'where are we up to with...', anything needing old mail. Emit research ALONE, never with a draft — Paul reviews the result first)
